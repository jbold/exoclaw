...
impl SessionRouter {
    ...
    pub fn resolve(&mut self, channel: &str, account: &str, peer: Option<&str>, guild: Option<&str>, team: Option<&str>) -> RouteResult {
        // Priority: peer > guild > team > account > channel > default
        let (agent_id, matched_by) = self.bindings.iter()
            .find_map(|b| {
                if let (Some(bp), Some(p)) = (&b.peer_id, peer) {
                    if bp == p { return Some((&b.agent_id, "binding.peer")); }
                }
                None
            })
            .or_else(|| self.bindings.iter().find_map(|b| {
                if let (Some(bg), Some(g)) = (&b.guild_id, guild) {
                    if bg == g { return Some((&b.agent_id, "binding.guild")); }
                }
                None
            }))
            .or_else(|| self.bindings.iter().find_map(|b| {
                if let (Some(bt), Some(t)) = (&b.team_id, team) {
                    if bt == t { return Some((&b.agent_id, "binding.team")); }
                }
                None
            }))
            .or_else(|| self.bindings.iter().find_map(|b| {
                if b.account_id.as_deref() == Some(account) && b.peer_id.is_none() && b.guild_id.is_none() {
                    return Some((&b.agent_id, "binding.account"));
                }
                None
            }))
            .or_else(|| self.bindings.iter().find_map(|b| {
                if b.channel.as_deref() == Some(channel) && b.account_id.is_none() && b.peer_id.is_none() {
